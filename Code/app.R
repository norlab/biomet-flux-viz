...
# January 2024
# Edits: Bree Norlander, Data Scientist
# bree.norlander@ubc.ca
# ----
...
ui <- page_sidebar(
  ...
  # App title ----
  title ='UBC Biometeorology Flux Visualization',
  
  # Sidebar panel for inputs ----
  sidebar = sidebar(
    
    # ADD SHORT DESCRIPTION
    p(tags$b("App Instructions"),
       align = 'left'),
    p(
      "Upload your Eddy-Covariance research site data below in '.csv' format. The data should present readings on a half-hourly or hourly basis and the file size should", 
      tags$b("not"), 
      "exceed 50MB. You may upload Stage Two data and Stage Three data for comparison. Stage Three data is",
      tags$b("not"),
      "required.", 
      tags$br(),
      tags$br(),
      "Data should be formatted to",
      tags$a(href = 'https://ameriflux.lbl.gov/', 'AmeriFlux'),
      "specifications and include 'TIMESTAMP_START' and 'TIMESTAMP_END' variables in", 
      tags$code("YYYYMMDDHHMM"), 
      "format.",
      align = 'left',
    ...
    fileInput(inputId = 'thrfile',
              label = 'Upload Third Stage csv Below (Optional)',
              buttonLabel = "Select File",
              multiple = TRUE,
              accept = '.csv')
    
  ), # end sidebar panel for data upload
  ...
  navset_card_underline(
    ...
    # Start orig code here
    
    # BEGIN ALL TAB SUBSETS
    
    nav_panel('Time Series',         # BEGIN TIME SERIES PANEL
             ...
             ## BEGINS W/ NULL CHOICES DUE TO NO FILE BEING UPLOADS
             fluidRow( column(6,
             ...
             shinycssloaders::withSpinner(plotlyOutput('timeseries'),
                                          type = getOption("spinner.type", default = 0),
                                          color = getOption("spinner.color", default = "dodgerblue")),
             
             p('Plot shows daily average of half-hourly or hourly data.', 
                align = 'center')
             
    ), # END TIME SERIES PANEL
    
    nav_panel('Diurnal Cycle',
    ...
    nav_panel('Density & Scatter Plots',      # BEGIN SCATTER PLOTS TAB
  ...
  observeEvent(inputs(), {
    ...
    if (is.null(input$secfile) == F) { # IF STATEMENT FOR WHEN THRSTAG FILE IS MISSING
      if (length(input$secfile$name) == 1) {
        ...
        ProSecFile <- data_processing(SelectedData$sec_clean, 'https://raw.githubusercontent.com/ubc-micromet/Biomet.net/main/R/data_visualization/Biomet_shiny/flux_variables.csv', 'Second')
        ...
        ProSecFile <- data_processing(CleanedData, 'https://raw.githubusercontent.com/ubc-micromet/Biomet.net/main/R/data_visualization/Biomet_shiny/flux_variables.csv', 'Second')
    ...
    if (is.null(input$thrfile) == F) { # IF STATEMENT FOR WHEN THRSTAG FILE IS MISSING
      if (length(input$thrfile$name) == 1) {
        ...
        ProThrFile <- data_processing(SelectedData$thr_clean, 'https://raw.githubusercontent.com/ubc-micromet/Biomet.net/main/R/data_visualization/Biomet_shiny/flux_variables.csv', 'Third')
        ...
        ProThrFile <- data_processing(ThrCleanedData, 'https://raw.githubusercontent.com/ubc-micromet/Biomet.net/main/R/data_visualization/Biomet_shiny/flux_variables.csv', 'Third')
